...
int Preemptive_Schedule(op_array, num, head)	int op_array[];  int num;  int head[];
{
  ...
  /* fprintf (Output, "[Preemptive_Schedule (num=%d)]\n", num); */
  ...
  /* fprintf (Output, "[Preemptive_Schedule: return max_low=%d]\n", max_low); */
  return(max_low);
}
...
int Compute_LowerBound ()
{
  ...
  /*fprintf (Output, "[Compute_LowerBound: begin]\n");*/
  ...
  for (i = 1; i <= NumOfMachines; ++i) {
     ...
     if ( max_lower_bound >= UpperBound ) {
        /*fprintf (Output, "[Compute_LowerBound: return max_lower_bound=%d, i=%d]\n", max_lower_bound, i);*/
