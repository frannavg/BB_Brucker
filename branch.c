...
static boolean CheckBefore(Op, block)	int Op;  struct List *block;
...
static boolean CheckAfter(Op, block)	int Op;  struct List *block;
...
static boolean SimpleLowerBound(Op, block, head, tail)	int Op;  struct List * block;  int head[];  int tail[];
...
static void InsertInOrder(Op, Direction)	int Op;  char Direction;
...
void Compute_BranchList ()
{
 ...
 /*fprintf (Output, "[Compute_BranchList: begin]\n");*/
 ...
 /*fprintf (Output, "[Compute_BranchList: end]\n");*/ 
}