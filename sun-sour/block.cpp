...
/* ************************************************************************* */
void Compute_Blocks() {
  BlockList *actual_block, *first_block, *helpblock;
  List *help;
  int num_of_elements,   mach_nr;
  actual_block = NIL;
  while (CriticalPath != NIL) {	/* Calculation of the first block */
    help = CriticalPath;
    mach_nr = OpData[CriticalPath->number].machine_nr;
    num_of_elements = 1;
    while (help->next != NIL && OpData[help->next->number].machine_nr == mach_nr) {
      help = help->next;
      num_of_elements++;
    }
    if (num_of_elements == 1)	/* no block */
      ...
      first_block = (BlockList *) malloc(sizeof(BlockList));
      first_block->elements = CriticalPath;
      first_block->last_of_block = help->number;
      first_block->fi_mi_la = 'f';
      first_block->next = NIL;
      first_block->elements = InsertBefore(first_block->elements, num_of_elements);
  ...
  while (CriticalPath != NIL) {	/* Calculation of the other blocks */
    ...
    if (num_of_elements == 1)	/* no block  */
      ...
      actual_block = (BlockList *) malloc(sizeof(BlockList));
      actual_block->elements = CriticalPath;
      actual_block->last_of_block = help->number;
      actual_block->next = NIL;
      actual_block->elements = InsertBefore(actual_block->elements, num_of_elements);
      CriticalPath = help->next;
      help->next = NIL;
      if (CriticalPath == NIL)
	actual_block->fi_mi_la = 'l';
      else
	actual_block->fi_mi_la = 'm';
  ...
  if (actual_block != NIL) {
    if (actual_block->fi_mi_la != 'l')
      first_block->fi_mi_la = 'm';
  } else
    first_block->fi_mi_la = 'l';
  
  helpblock = SonNode->blocks;
  
  while (helpblock != NIL) {
    helpblock->elements =
      Delete(helpblock->elements, helpblock->elements->number);
    helpblock = helpblock->next;
  }
}
