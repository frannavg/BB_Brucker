...
/* solving the primal or the dual problem for all operations on
   machine mach_no */
static int Jackson_Immediate_Selection(int mach_no, int head[], int tail[]) {
  int set_O[MaxOpProMachine], num_of_O,
    ...
    rest_pos, old_pos,  differenz, max_low = 0, new_low;
  register int i, Op, num;
  bool computed[MaxNumOfOperations + 1];
  List *help;
  num = 0;
  ...
  while (rest_pos <= num) {
	...
	if (computed[tail_sort[i]]) {
	  if (sum_processtime + tail[tail_sort[i]] >= UpperBound)
	    ...
	    computed[tail_sort[i]] = false, sum_processtime -=
	      process[tail_sort[i]];
	  }
	}
	i++;
...
static bool Select(int mach_no) {
  List *help1, *help2;
  bool new_arcs = false;
  int sum_process;
  ...
  while (help1 != NIL && help1->next != NIL) {
    ...
    while (help2 != NIL) {
      ...
      if (head[Op1] + sum_process + tail[Op2] >= UpperBound && !Member(DisjArcs->succ[Op2], Op1)) {
	...
	new_arcs = true;
      ...
      if (head[Op2] + sum_process + tail[Op1] >= UpperBound && !Member(DisjArcs->succ[Op1], Op2)) {
