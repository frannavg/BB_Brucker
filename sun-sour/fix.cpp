...
void Fix_Disjunctions(int MoveOp, char Direc) {
  bool ready, found, just_found, look_at_first_block;
  List *help, *elements_of_first_block;
  BlockList *helpblock;
  int last, first, i;
  char type;
  ...
  ready = found = just_found = look_at_first_block = false;
   while (helpblock != NIL && !ready) {
	...
	if (Direc == 'b') {
	  while (help != NIL) {
	    if (help->number != last && !Member(DisjArcs->succ[help->number], last)) {
	      DisjArcs->succ[help->number] = Insert(DisjArcs->succ[help->number], last);
	      DisjArcs->pred[last] = Insert(DisjArcs->pred[last], help->number);
	      SonNode->num_of_succ[help->number]++;
	      SonNode->num_of_pred[last]++;
	    }
	    if (help->number != MoveOp && !Member(DisjArcs->succ[MoveOp], help->number)) {
	      DisjArcs->succ[MoveOp] = Insert(DisjArcs->succ[MoveOp], help->number);
	      DisjArcs->pred[help->number] = Insert(DisjArcs->pred[help->number], MoveOp);
	  ...
	  ready = true;
	  while (help != NIL) {
	    if (help->number != MoveOp && !Member(DisjArcs->pred[MoveOp], help->number)) {
	      DisjArcs->pred[MoveOp] = Insert(DisjArcs->pred[MoveOp], help->number);
	      DisjArcs->succ[help->number] = Insert(DisjArcs->succ[help->number], MoveOp);
	      SonNode->num_of_succ[help->number]++;
	...
	if (Direc == 'b') {
	  ...
	  while (help != NIL) {
	    if (!Member(DisjArcs->succ[first], help->number)) {
	      DisjArcs->succ[first] = Insert(DisjArcs->succ[first], help->number);
	      DisjArcs->pred[help->number] = Insert(DisjArcs->pred[help->number], first);
	      SonNode->num_of_pred[help->number]++;
